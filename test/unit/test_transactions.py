...
def test_add_transaction_invalid_type(db):
    ...
    contact_id = db.contacts.get_contacts()["data"][0]["id"]
    res = db.transactions.add_transaction(contact_id, "loan", 30, "2025-08-19", "Loan")
...
def test_add_transaction_negative_amount(db):
    ...
    db.contacts.add_contact("Eve")
    contact_id = db.contacts.get_contacts()["data"][0]["id"]
    res = db.transactions.add_transaction(contact_id, "credit", -10, "2025-08-19", "Error")
    assert not res["success"]
...
def test_transaction_contact_id_invalid(db):
    ...
    res = db.transactions.add_transaction(9999, "debit", 10, "2025-08-19", "Error")
    assert not res["success"]
